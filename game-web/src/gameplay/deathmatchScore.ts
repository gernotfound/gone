...
type ScoreRow = { slot: number; name: string; kills: number; deaths: number; damage: number; headshots: number };
...
function rowFor(slot: number): ScoreRow {
  let row = scores.get(slot);
  if (!row) { row = { slot, name: playerName(slot), kills: 0, deaths: 0, damage: 0, headshots: 0 }; scores.set(slot, row); }
  else row.name = playerName(slot);
  return row;
}
function resetMatch(): void { scores.clear(); winnerSlot = null; matchStartedAt = performance.now(); render(); }
function ensureUi(): void {
  if (!document.getElementById('gone-deathmatch-style')) {
    const style = document.createElement('style');
    style.id = 'gone-deathmatch-style';
    style.textContent = `#gone-dm-compact{position:fixed;right:16px;top:16px;z-index:68;pointer-events:none;min-width:170px;padding:8px 10px;border-radius:9px;background:rgba(2,6,23,.78);border:1px solid rgba(34,211,238,.3);color:#cbd5e1;font:800 10px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace;box-shadow:0 0 16px rgba(34,211,238,.1)}#gone-dm-board{position:fixed;left:50%;top:18%;transform:translateX(-50%);z-index:84;min-width:min(620px,88vw);padding:14px 16px;border-radius:12px;background:rgba(2,6,23,.94);border:1px solid rgba(34,211,238,.45);color:#cbd5e1;font:800 11px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;box-shadow:0 0 30px rgba(8,145,178,.18);display:none;pointer-events:none}#gone-dm-board table{width:100%;border-collapse:collapse}#gone-dm-board th,#gone-dm-board td{padding:5px 7px;text-align:right;border-bottom:1px solid rgba(148,163,184,.12)}#gone-dm-board th:first-child,#gone-dm-board td:first-child{text-align:left}#gone-dm-winner{position:fixed;left:50%;top:22%;transform:translateX(-50%);z-index:85;padding:10px 18px;border-radius:10px;background:rgba(127,29,29,.9);border:1px solid #fb7185;color:#fff;font:900 15px ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.12em;display:none;pointer-events:none;box-shadow:0 0 24px rgba(251,113,133,.35)}`;
...
function sortedRows(): ScoreRow[] { return [...scores.values()].sort((a, b) => b.kills - a.kills || a.deaths - b.deaths || b.damage - a.damage || a.slot - b.slot); }
function render(): void {
  ...
  const me = meSlot === null ? null : rowFor(meSlot);
  const leader = sortedRows()[0] ?? me;
  compactRoot!.innerHTML = `<div style="color:#67e8f9;font-size:11px;letter-spacing:.12em">DEATHMATCH // 20</div><div>TU <b style="color:#f8fafc">${me?.kills ?? 0}K / ${me?.deaths ?? 0}D</b></div><div>LEADER <b style="color:#fbbf24">${escapeHtml(leader?.name ?? '—')} ${leader?.kills ?? 0}</b></div><div style="color:#64748b;font-size:9px">TAB · CLASSIFICA</div>`;
  const rows = sortedRows();
  boardRoot!.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><span style="color:#67e8f9;font-size:13px">G.O.N.E. DEATHMATCH</span><span style="color:#64748b">PRIMO A ${TARGET_KILLS}</span></div><table><thead><tr><th>GIOCATORE</th><th>K</th><th>D</th><th>DMG</th><th>HS</th></tr></thead><tbody>${rows.map((row) => `<tr${row.slot === meSlot ? ' style="color:#67e8f9"' : ''}><td>${escapeHtml(row.name)}</td><td>${row.kills}</td><td>${row.deaths}</td><td>${Math.round(row.damage)}</td><td>${row.headshots}</td></tr>`).join('')}</tbody></table>`;
  boardRoot!.style.display = boardVisible ? 'block' : 'none';
  if (winnerSlot !== null) { winnerRoot!.textContent = `${playerName(winnerSlot)} · TARGET ${TARGET_KILLS} RAGGIUNTO`; winnerRoot!.style.display = 'block'; }
  else winnerRoot!.style.display = 'none';
}
function ingest(detail: CombatHitEventDetail): void {
  const hit = detail.hit;
  const shooter = rowFor(hit.shooterSlot);
  const victim = rowFor(hit.victimSlot);
  if (!hit.isShieldBlocked) { shooter.damage += Math.max(0, Number(hit.damage) || 0); if (hit.isHeadshot) shooter.headshots += 1; }
  if (hit.isFatalKill || hit.isFatal) { shooter.kills += 1; victim.deaths += 1; if (winnerSlot === null && shooter.kills >= TARGET_KILLS) winnerSlot = shooter.slot; }
  render();
}
function sessionWatch(): void { const next = currentSessionKey(); if (next !== sessionKey) { sessionKey = next; resetMatch(); } render(); }
export function startDeathmatchScore(): void {
  ...
  ensureUi(); sessionKey = currentSessionKey();
  window.addEventListener('gone-hit-confirmed', ((event: CustomEvent<CombatHitEventDetail>) => ingest(event.detail)) as EventListener);
  ...
  window.setInterval(sessionWatch, 500); render();
  (window as any).goneDeathmatch = { snapshot: () => ({ targetKills: TARGET_KILLS, winnerSlot, localSlot: localSlot(), elapsedMs: performance.now() - matchStartedAt, rows: sortedRows().map((row) => ({ ...row })) }), reset: resetMatch };
}
