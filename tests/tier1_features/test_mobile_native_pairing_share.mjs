...
export async function run(suite) {
  suite.test('Direct WebRTC pairing offers native device sharing without changing session ownership', () => {
    ...
    assert(source.includes("snapshot.inviteKind === 'host-link' || snapshot.inviteKind === 'host-room'"), 'sharing must recognize both manual and automatic controller-owned host invitations');
    assert(source.includes("snapshot.inviteKind !== 'guest-answer'"), 'sharing must still reject non-invite/non-answer controller states');
    assert(source.includes("url: snapshot.inviteValue"), 'host flow must share the canonical invite URL');
    ...
    assert(source.includes("button.textContent = hostInvite ? 'CONDIVIDI INVITO' : 'INVIA RISPOSTA'"), 'share action must explain whether it carries the invitation or answer');
    ...
    assert(source.includes('multiplayerSessionController.startDirectGuest(directOfferCode'), 'existing manual guest negotiation path must remain intact');
    assert(source.includes('multiplayerSessionController.startFirestoreGuest(options.roomId'), 'automatic room negotiation must remain controller-owned');
  });
}
